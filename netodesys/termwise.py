...
class TermwiseDynamicalMeta(abc.ABCMeta):
    ...
    def __new__(mcs, name, bases, attrs, **kwargs):
        graph_bases = [base for base in bases if issubclass(base, nx.Graph)]
        if graph_bases:
...
class TermwiseUndirected(object, metaclass=abc.ABCMeta):

    def rhs(self):
        for u in self:
            eq = np.array(self.node_term(u))
            for v in self.neighbors(u):
                eq += np.array(self.source_term(u, v))
            yield u, eq


class TermwiseDirected(object, metaclass=abc.ABCMeta):
